...
func New(sectors SectorProvider, cfg *Config) (*Sealer, error) {
	...
	sb := &Sealer{
		...
		generatePoSt: func(minerID abi.ActorID, privateSectorInfo ffi.SortedPrivateSectorInfo, randomness abi.PoStRandomness, proofsStr string, isWinningPoSt bool) ([]abi.PoStProof, error) {
			if isWinningPoSt {
				return ffi.GenerateWinningPoSt(minerID, privateSectorInfo, randomness, proofsStr)
			} else {
				return ffi.GenerateWindowPoSt(minerID, privateSectorInfo, randomness, proofsStr)
			}
		},
	}
